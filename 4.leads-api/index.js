...
app.post('/', function(req, res) {
    ...
    if (invalidInputs.length === 0) {
        connection.query(`INSERT INTO leads (first_name, last_name, phone_number, email_address) VALUES ("${firstName}", "${lastName}", "${phoneNumber}", "${emailInput}")`, function(error, results, fields) {
...
app.post('/getAllLeads', function(req, res) {
    ...
    const query = `SELECT * from leads ORDER BY ${sorted_column || 'id'} ${isDesc ? 'DESC' : 'ASC'}`;
    console.log(query);
